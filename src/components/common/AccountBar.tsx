...
import { LogIn, LogOut, Cloud, CloudOff, Loader2, AlertTriangle, Check } from 'lucide-react';
...
export const AccountBar: React.FC = () => {
  const { user, loading, login, logout, conflict, resolveConflict } = useAuth();
          ...
          <div className="w-full max-w-sm rounded-2xl border border-[#c5a059]/40 bg-[#120a1a] p-5 text-center shadow-2xl">
            ...
            <div className="mt-5 space-y-2">
              <button
                ...
                className="w-full rounded-xl border border-[#332244] bg-[#1a1026] px-4 py-2.5 text-sm font-semibold text-[#e5c158] hover:border-[#c5a059]"
              >
                Use my cloud progress
              </button>
              <button
                data-testid="conflict-use-local-btn"
                onClick={() => void resolveConflict('use_anonymous')}
                className="w-full rounded-xl border border-rose-900/60 bg-rose-950/40 px-4 py-2.5 text-sm font-semibold text-rose-200 hover:border-rose-600"
              >
