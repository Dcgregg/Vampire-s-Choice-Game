...
describe('navigation', () => {
  it('advances to the target scene and records history', () => {
    ...
    expect(res.returnToLanding).toBe(false);
  ...
  it('returnToLanding leaves progress untouched', () => {
    const state = baseState();
    const res = navigate(state, { id: 'c', text: 't', nextSceneId: 'b1_c3_s3', returnToLanding: true }, getSceneById);
