"""Deterministic proof-mutation races against disposable MongoDB only."""
import os
...
class MutateBeforeFence:
    ...
    def __init__(self, collection, ledger_id, mutation=None):
        self.collection = collection
        self.ledger_id = ledger_id
        self.mutation = mutation or {"$set": {"appliedEventIds.1": "foreign"}}
        self.mutated = False
    ...
    async def update_one(self, *args, **kwargs):
        if not self.mutated:
            ...
            await self.collection.update_one(
                {"_id": self.ledger_id}, self.mutation,
            )
...
@pytest.mark.asyncio
async def test_unrelated_ledger_write_after_read_cannot_finalise_event():
    uri = os.environ.get("TEST_MONGO_URI")
    if not uri:
        pytest.skip("TEST_MONGO_URI required; never use production MongoDB")
    client = AsyncIOMotorClient(uri, serverSelectionTimeoutMS=5000)
    db = client[f"p6b_final_other_{uuid4().hex}"]
    try:
        await client.admin.command("ping")
        ledger_id = uuid4().hex
        event = {"_id": uuid4().hex, "ledgerId": ledger_id, "eventId": "own",
        ...
        await db.ledgers.insert_one({
            "_id": ledger_id, "progressionRevision": 1,
            "appliedEventIds": {"1": "own"}, "coins": {"confirmed": 10},
        })
        await db.events.insert_one(dict(event))
        intercepted = MutateBeforeFence(
            db.ledgers, ledger_id, {"$set": {"coins.confirmed": 99}},
        )
        with pytest.raises(AttributionUnproven):
            await finalise_attributed_event(
                intercepted, db.events, event=event, payload_hash="digest",
        ...
        ledger = await db.ledgers.find_one({"_id": ledger_id})
        stored = await db.events.find_one({"_id": event["_id"]})
        assert ledger["coins"]["confirmed"] == 99
        assert ledger["appliedEventIds"]["1"] == "own"
        assert "attributionProofFence" not in ledger
        assert stored["status"] == "committing"
        assert "resultRevision" not in stored
    finally:
        await client.drop_database(db.name)
